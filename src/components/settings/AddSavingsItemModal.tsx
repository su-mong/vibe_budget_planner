...
import type { SavingsItem } from '../../types/budget';
import { Modal } from '../shared/Modal';

export function AddSavingsItemModal() {
  const { state, dispatch } = useBudget();
  const [name, setName] = useState('');

  const isOpen = state.modalState.type === 'addSavingsItem';
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('savings_items')
        .insert({ user_id: user.id, name: name.trim(), order: state.savingsItems.length })
        .select()
        .single();
      ...
      console.error('Failed to add savings item:', err);
    }
  };

  return (
    <Modal open={isOpen} onClose={handleClose} title="저축 항목 추가">
